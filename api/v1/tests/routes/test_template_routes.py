...
test_template_data = {
    ...
    "template_tags": ["ai", "ml", "data-science"],
...
def created_template_id():
    ...
    test_template_data["template_name"] = (
        "Cookiecutter DS Public For Testing" + random_string()
    )
    response = client.post("/v1/templates/", json=test_template_data)
    assert response.status_code == 200
    template_data = response.json()
    yield template_data["tid"][
        "value"
    ]  # This will be used as the template_id in later tests
...
def test_get_templates():
    ...
    response = client.get(
        "/v1/templates/", params={"user_id": "653910690eccd12c6b60c3f3"}
    )
    assert response.status_code == 200
    assert isinstance(response.json(), list)
...
def created_template_private_id():
    # Setup: Create a template
    private_template_sample["template_name"] = (
        "Cookiecutter DS Private For Testing" + random_string()
    )
    response = client.post("/v1/templates/", json=private_template_sample)
    assert response.status_code == 200
    template_data = response.json()
    ...
    # Teardown: Delete the template
    client.delete(
        f"/v1/templates/{template_data['tid']['value']}",
        params={"user_id": "653910690eccd12c6b60c3f3"},
    )
...
def test_get_private_template(created_template_private_id):
    response = client.get(
        f"/v1/templates/{created_template_private_id}",
        params={"user_id": "653910690eccd12c6b60c3f3"},
    )
    assert response.status_code == 200
